...
namespace FAMAndIMS.Data.Services.GlobalSettingServices.UnitServices
{
    public class UnitServices : IUnitServices
    {
        ...
        public UnitServices(DBConnectionManager dbConnectionManager, IHostingEnvironment hostEnv, IConfiguration config, IMapper mapper, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<PagedResponse<UnitListModel>> GetUnitList(UnitDto unitDto)
        {
            try
            {
                using var db = _dbConnectionManager.GetConnection();
                var p = unitDto.PrepareDynamicParameters();
                db.Open();
                var result = await db.QueryMultipleAsync("[dbo].[usp_GetUnitList]", param: p, commandType: CommandType.StoredProcedure);
                var unitList = await result.ReadAsync<UnitListModel>();
                var pagedInfo = await result.ReadFirstAsync<PagedInfo>();
                var mappedData = _mapper.Map<PagedResponse<UnitListModel>>(pagedInfo);
                mappedData.Items = unitList;
                db.Close();
                return mappedData;
            }
            catch (Exception)
        ...
        public async Task<SpResponseMessage> SaveUnit(UnitVM unitVM)
        {
            try
            {
                ...
                using var db = _dbConnectionManager.GetConnection();
                var p = UnitModel.PrepareDynamicParameters();
                ...
                var result = await db.ExecuteAsync("[dbo].[usp_IUDUnit]", param: p, commandType: CommandType.StoredProcedure);
        ...
        public async Task<UnitVM> GetUnitById(int Id)
        {
            try
            {
                using var db = _dbConnectionManager.GetConnection();
                ...
                db.Open();
                p.Add("@UnitId", Id);
                var data = await db.QuerySingleAsync<UnitVM>(sql: "[dbo].[usp_GetUnitById]", param: p, commandType: CommandType.StoredProcedure);
                db.Close();
                return data;
            }
            catch (Exception)
        ...
        public async Task<SpResponseMessage> DeleteUnit(int Id)
        {
            try
            {
                using var db = _dbConnectionManager.GetConnection();
